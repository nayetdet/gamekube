...
public class UserService {
    ...
    public UserResponse createUser(UserCreateRequest request) {
        if (userRepository.existsByEmail(request.email())) {
            throw new UserAlreadyExistsException("Já existe um usuário cadastrado com o e-mail: " + request.email());
        }
        if (userRepository.existsByUsername(request.username())) {
            throw new UserAlreadyExistsException("Já existe um usuário cadastrado com o username: " + request.username());
        }
    ...
    public UserResponse getUserByKeycloakId(String keycloakId) {
        User user = userRepository.findByKeycloakId(keycloakId)
            .orElseThrow(() -> new ResourceNotFoundException("Usuário não encontrado com o Keycloak ID: " + keycloakId));
        return UserResponse.fromEntity(user);
    }
    ...
    public UserResponse updateUser(UUID id, UserUpdateRequest request) {
        ...
        if (userRepository.existsByUsernameAndIdNot(request.username(), id)) {
            throw new UserAlreadyExistsException("O username " + request.username() + " já está em uso por outro usuário");
        }

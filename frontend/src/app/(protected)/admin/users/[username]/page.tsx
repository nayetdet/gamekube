...
export default function UserDetailsPage() {
  ...
  if (loading) return <LoadingState label="Carregando usuário…" />;
  if (error)
    return (
      ...
      <div className="mt-9 grid gap-6 lg:grid-cols-[1.35fr_0.85fr]">
        <section className="rounded-2xl border border-white/10 bg-slate-950/30 p-6 sm:p-8">
          <UserForm
            ...
            user={user}
            onSaved={(updatedUsername) =>
              updatedUsername === username
                ? reload()
                : router.replace(
                    `/admin/users/${encodeURIComponent(updatedUsername)}`,
                  )
            }
          />
        </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-sm">
            ...
            <dl className="mt-4 space-y-3 text-slate-400">
              <div>
                ...
                <dd className="mt-1 break-all text-slate-200">{user.id}</dd>
              </div>
              <div>
                <dt className="text-xs uppercase tracking-wider text-slate-500">
                  Criado em
